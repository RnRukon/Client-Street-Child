...
const HomePageChild = () => {
    ...
    const filterChild = childList?.result?.filter(d => age(new Date(d?.dateOfBirth)) >= Number(ageForFilterGreaterThan))


    const items = filterChild?.filter(d => d?.status !== "pending" || d?.status !== "rejected");
    ...
    return (
        ...
        <div className='bg-gradient-to-br from-slate-200 via-sky-500 to-slate-200'>
            ...
            <div className=' container mx-auto  pb-24 p-3  '>
                <h1 className=' font-extrabold  font-serif text-3xl md:text-6xl  text-yellow-300   py-10 text-center'>Street Child</h1>
                <div className=''>
